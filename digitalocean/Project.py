...
class Project(BaseAPI):
    ...
    def load(self):
        ...
        project = self.get_data(f"projects/{self.id}")
    ...
    def set_as_default_project(self):
        ...
        return self.get_data(f"projects/{self.id}", type=PUT, params=data)
    ...
    def create_project(self):
        ...
        if data := self.get_data("projects", type=POST, params=data):
    ...
    def delete_project(self):
        data = {}
        return self.get_data(f"projects/{self.id}", type=DELETE, params=data)
    ...
    def update_project(self, **kwargs):
        ...
        data = {
            ...
            'purpose': kwargs.get("purpose", self.purpose),
            'environment': kwargs.get("environment", self.environment),
            'is_default': kwargs.get("is_default", self.is_default),
        }
        update_response = self.get_data(f"projects/{self.id}", type=PUT, params=data)
        for attr in update_response['project'].keys():
            setattr(self, attr, update_response['project'][attr])

    def get_all_resources(self):
        project_resources_response = self.get_data(f"projects/{self.id}/resources")
        project_resources = project_resources_response['resources']
        self.resources = [i['urn'] for i in project_resources]
    ...
    def assign_resource(self, resources):
        ...
        return self.get_data(f"projects/{self.id}/resources", type=POST, params=data)
